...
import { birimWhere } from "./birim";

/** Kurumun onaylı hesap izlerini motor satırlarına dönüştürür. birim verilirse o birime daraltır. */
export async function getEmissionRows(orgId: string, unitId?: string): Promise<EmissionRow[]> {
  const records = await prisma.emissionRecord.findMany({
    where: { activityData: { facility: { orgId, ...birimWhere(unitId) } } },
    select: {
      scope: true,
      tCO2e: true,
      activityData: {
...
export async function getMissingData(orgId: string, year: number, month: number, unitId?: string) {
  const facilities = await prisma.facility.findMany({
    where: { orgId, ...birimWhere(unitId) },
...
export async function getLatestPeriod(orgId: string, unitId?: string): Promise<{ year: number; month: number } | null> {
  const last = await prisma.activityData.findFirst({
    where: { facility: { orgId, ...birimWhere(unitId) } },
...
export async function getPendingCount(orgId: string, unitId?: string): Promise<number> {
  return prisma.activityData.count({ where: { status: "TASLAK", facility: { orgId, ...birimWhere(unitId) } } });
}
