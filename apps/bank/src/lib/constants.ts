...
export const CATEGORIES: readonly CategoryMeta[] = [
  ...
  // envanter kataloğu (M12)
  { code: "SOGUTUCU_GAZ",    label: "soğutucu gaz kaçağı",  unit: "kg",        scope: 1 },
] as const;
...
export const ROLE_LABELS: Record<Role, string> = {
  ...
  BANKA_ADMIN: "banka yöneticisi",
  BANKA_ANALIST: "banka analisti",
  KLEAF_ADMIN: "kleaf denetim yöneticisi",
  KLEAF_DENETCI: "kleaf denetçisi",
};

export const ORG_TYPES = ["BELEDIYE", "KARBON_BANK", "KLEAF", "SANAYI", "KAMU"] as const;
...
export const ORG_TYPE_LABELS: Record<OrgType, { product: string; label: string }> = {
  ...
  KARBON_BANK: { product: "KarbonBank", label: "karbon bankası" },
  KLEAF: { product: "Kleaf Denetim", label: "kleaf denetim" },
  SANAYI: { product: "KarbonBank", label: "sanayi" },
  KAMU: { product: "KarbonKent Kurumsal", label: "kamu" },
};
...
export const ISO_CATEGORIES = ["CAT1", "CAT2", "CAT3", "CAT4", "CAT1_2", "CAT3_4", "KENT"] as const;
export type IsoCategory = (typeof ISO_CATEGORIES)[number];
export const ISO_CATEGORY_LABELS: Record<IsoCategory, string> = {
  CAT1: "Kategori 1 — doğrudan",
  CAT2: "Kategori 2 — satın alınan enerji",
  CAT3: "Kategori 3 — ulaşım",
  CAT4: "Kategori 4 — ürün ve hizmetler",
  CAT1_2: "Kategori 1-2 — karma",
  CAT3_4: "Kategori 3-4 — karma",
  KENT: "kent ölçeği — envanter dışı izleme",
};

export const INVENTORY_MODES = ["HESAPLANABILIR", "IZLEME"] as const;
export type InventoryMode = (typeof INVENTORY_MODES)[number];
export const INVENTORY_MODE_LABELS: Record<InventoryMode, string> = {
  HESAPLANABILIR: "hesaplanabilir", IZLEME: "izleme",
};

/* ── karbon bankası (M13) ── */

export const CREDIT_STANDARDS = ["GOLD_STANDARD", "VCS", "ULUSAL"] as const;
export type CreditStandard = (typeof CREDIT_STANDARDS)[number];
export const CREDIT_STANDARD_LABELS: Record<CreditStandard, string> = {
  GOLD_STANDARD: "Gold Standard", VCS: "Verra VCS", ULUSAL: "ulusal program",
};

export const CREDIT_STATUS = ["TALEP", "BANKA_ONAY", "TRANSFER", "RED", "IPTAL", "DENETIM_ASKI"] as const;
export type CreditStatus = (typeof CREDIT_STATUS)[number];
export const CREDIT_STATUS_LABELS: Record<CreditStatus, string> = {
  TALEP: "talep edildi", BANKA_ONAY: "banka onayladı", TRANSFER: "transfer edildi",
  RED: "reddedildi", IPTAL: "iptal edildi", DENETIM_ASKI: "denetim askıda",
};

export const CREDIT_POOL_TYPES = ["SATIS", "BANKA_REZERV"] as const;
export type CreditPoolType = (typeof CREDIT_POOL_TYPES)[number];
export const CREDIT_POOL_TYPE_LABELS: Record<CreditPoolType, string> = {
  SATIS: "satış havuzu", BANKA_REZERV: "banka rezerv (öz-mahsup)",
};

export const COMPLIANCE_FLAG_TYPES = ["CIFTE_SAYIM", "BUFFER_ALTI", "FIYAT_ANOMALI", "ASIRI_YOGUNLASMA", "BRUT_ASIM"] as const;
export type ComplianceFlagType = (typeof COMPLIANCE_FLAG_TYPES)[number];
export const COMPLIANCE_FLAG_TYPE_LABELS: Record<ComplianceFlagType, string> = {
  CIFTE_SAYIM: "çifte sayım", BUFFER_ALTI: "buffer altı satış",
  FIYAT_ANOMALI: "fiyat anomalisi", ASIRI_YOGUNLASMA: "aşırı yoğunlaşma",
  BRUT_ASIM: "brüt emisyon aşımı",
};

export const COMPLIANCE_SEVERITY = ["DUSUK", "ORTA", "YUKSEK"] as const;
export type ComplianceSeverity = (typeof COMPLIANCE_SEVERITY)[number];
export const COMPLIANCE_SEVERITY_LABELS: Record<ComplianceSeverity, string> = {
  DUSUK: "düşük", ORTA: "orta", YUKSEK: "yüksek",
};

export const AUDIT_DECISION_TYPES = ["ONAY", "ASKI", "ITIRAZ"] as const;
export type AuditDecisionType = (typeof AUDIT_DECISION_TYPES)[number];
export const AUDIT_DECISION_TYPE_LABELS: Record<AuditDecisionType, string> = {
  ONAY: "onay", ASKI: "askıya al", ITIRAZ: "itiraz",
};
