...
export async function middleware(req: NextRequest) {
  ...
  // SUPER_ADMIN banka portalında yer almaz — denetim portalına gönder
  if (session.role === "SUPER_ADMIN" && !pathname.startsWith("/api/")) {
    return NextResponse.redirect("http://localhost:3300/");
  }
  return NextResponse.next();
}
