...
async function seedEnvanterVeBanka(passwordHash: string) {
  ...
  const c = {
    ...
    kullanici: await prisma.user.count({ where: { orgId: { in: [ibb.id, banka.id, kleaf.id] } } }),
  };
  console.log(`✓ envanter+banka+kleaf tamam — ${c.kalem} kalem, ${c.veri} İBB kaydı, ${c.bankaVeri} banka kaydı, ${c.izleme} izleme, ${c.havuz} havuz, ${c.islem} işlem, ${c.kullanici} kullanıcı`);
}
