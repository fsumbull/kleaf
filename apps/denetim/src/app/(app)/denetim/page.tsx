...
export default async function DenetimPage({ searchParams }: {
  ...
  const session = await requireSession(["SUPER_ADMIN", "SISTEM_YONETICISI", "IKLIM_MERKEZI", "KLEAF_ADMIN", "KLEAF_DENETCI"]);
  const { org } = await getScope();
  // KLEAF (denetim otoritesi) tüm kurumlardaki kayıtları görür — birleşik denetim izi
  const isSuper = session.role === "SUPER_ADMIN" || org.type === "KLEAF";
