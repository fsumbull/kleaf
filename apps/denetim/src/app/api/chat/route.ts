...
// Denetim asistanı araçsız (yalnız soru-cevap) çalışır → küçük ve hızlı 3b modeli yeterli
...
function toolsFor(orgType: string) {
  if (orgType === "KARBON_BANK") return BANKA_TOOLS;
  if (orgType === "BELEDIYE") return BELEDIYE_TOOLS;
  // KLEAF (denetim otoritesi) ve diğer tipler: araçsız hızlı soru-cevap modu
  return [];
}
...
async function ollamaChat(messages: ChatMsg[], tools: unknown[]): Promise<ChatMsg> {
  const res = await fetch(`${OLLAMA_URL}/api/chat`, {
    ...
    body: JSON.stringify({
      model: OLLAMA_MODEL,
      messages,
      // araç yoksa hiç gönderme (model daha hızlı düz yanıt verir)
      ...(tools.length ? { tools } : {}),
      stream: false,
      // modeli 30 dk bellekte tut — cold-start gecikmesini önler
      keep_alive: "30m",
      options: {
        temperature: 0.2,
        top_p: 0.9,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const kurumTuru =
      org.type === "KARBON_BANK" ? "karbon bankası" :
      org.type === "BELEDIYE" ? "belediye" :
      org.type === "KLEAF" ? "ulusal denetim otoritesi (Kleaf Denetim)" :
      org.type.toLowerCase();
    const kapsamSatiri = [
      `Kurum: ${org.name} (${kurumTuru}).`,
      ...
      org.type === "KARBON_BANK"
        ...
        : org.type === "KLEAF"
          ? "Bu bir ulusal denetim otoritesidir: tüm belediye ve bankaların uyum, kredi ve envanter verilerini gözetler. İşlem aracı yoktur; kullanıcının sorularını platform bilgisiyle kısa ve net yanıtla."
          : "",
    ].filter(Boolean).join("\n");
