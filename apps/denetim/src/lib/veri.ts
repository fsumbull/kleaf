 ...
 *  Envanter kalemine bağlı kayıtlarda kuruma özel kalem faktörü (customFactorKgCO2e) önceliklidir. */
export async function approveActivity(activityId: string, orgId: string) {
  const act = await prisma.activityData.findUnique({
    where: { id: activityId },
    include: { inventoryItem: { select: { id: true, name: true, customFactorKgCO2e: true } } },
  });
  ...
  if (act.inventoryItem?.customFactorKgCO2e != null) {
    // kuruma özel kalem faktörü — kütüphane faktörünü geçersiz kılar
    const f = act.inventoryItem.customFactorKgCO2e;
    tCO2e = kgToTons(computeKgCO2e(cat, act.amount, f));
    snapshot = JSON.stringify({
      factorId: null,
      kgCO2ePerUnit: f,
      unit: act.unit,
      source: `kuruma özel kalem faktörü — ${act.inventoryItem.name}`,
      year: act.year,
      orgSpecific: true,
      inventoryItemId: act.inventoryItem.id,
    });
  } else {
    ...
    tCO2e = kgToTons(computeKgCO2e(cat, act.amount, factor.kgCO2ePerUnit));
    snapshot = JSON.stringify({
      factorId: factor.id,
      ...
      ...(act.inventoryItem ? { inventoryItemId: act.inventoryItem.id } : {}),
    });
  }
