...
export async function getScope(): Promise<Scope> {
  ...
  if (session.role === "SUPER_ADMIN") {
    ...
    // Denetim uygulamasında süper admin her zaman KLEAF perspektifinden başlar
    // (kurum bazlı inceleme /kurumlar/[id] üzerinden yapılır)
    const kleafOrg = orgs.find((o) => o.type === "KLEAF");
    orgId = kleafOrg?.id ?? orgs[0]?.id ?? null;
  }
