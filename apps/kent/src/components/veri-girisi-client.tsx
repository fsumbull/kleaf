...
export interface KalemOpt { id: string; name: string; unitName: string; dataUnit: string; categoryCode: string | null }
...
export function VeriGirisiClient({ rows, facilities, vehicles, kalemler, canEdit, canApprove, canUnitApprove, year, years, page, pages, total }: {
  rows: VeriRow[];
  ...
  return (
    <div className={pending ? "opacity-60 transition" : "transition"}>
      ...
      {modal && (
        <RecordModal
          facilities={facilities} vehicles={vehicles} kalemler={kalemler} year={year}
...
function RecordModal({ facilities, vehicles, kalemler, year, initial, prefill, lastRow, onClose, onSaved, setError, setConflict }: {
  facilities: { id: string; name: string }[];
  vehicles: VehicleOpt[];
  kalemler: KalemOpt[];
  year: number;
  ...
  const seciliKalem = kalemler.find((k) => k.id === kalemId);

  function kalemSec(id: string) {
    setKalemId(id);
    const k = kalemler.find((x) => x.id === id);
    if (k?.categoryCode) setValue("category", k.categoryCode, { shouldValidate: true });
  }
  ...
  async function onSubmit(v: FormValues) {
    ...
    const res = await fetch(isEdit ? `/api/veri/${initial.id}` : "/api/veri", {
      ...
      body: JSON.stringify(isEdit
        ...
        : { ...v, vehicleId: v.vehicleId || null, documentRef: v.documentRef || null, inventoryItemId: kalemId || null }),
    });
    if (!res.ok) {
      const data = await res.json().catch(() => null);
  ...
  return (
    <ModalShell title={initial ? "Kaydı düzenle" : "Yeni faaliyet kaydı"} onClose={onClose}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
        ...
        {!initial && kalemler.length > 0 && (
          <Field label="envanter kalemi (isteğe bağlı — kategoriyi kalemden çözer)">
            <select value={kalemId} onChange={(e) => kalemSec(e.target.value)} className={inputCls}>
              <option value="">— kalem seçilmedi —</option>
              {kalemler.map((k) => (
                <option key={k.id} value={k.id}>{k.unitName} · {k.name}</option>
              ))}
            </select>
          </Field>
        )}
        {seciliKalem && (
          <p className="-mt-2 text-[11px] text-ink/45">
            kalem birimi: <b>{seciliKalem.dataUnit}</b> — kayıt bu kaleme bağlanır, onayda kalem faktörü önceliklidir
          </p>
        )}
        <Field label="2 · kategori" error={errors.category?.message}>
          <select {...register("category")} className={inputCls} disabled={!!initial || !!kalemId}>
