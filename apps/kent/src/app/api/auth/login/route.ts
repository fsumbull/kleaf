...
export async function POST(req: Request) {
  ...
  const user = await prisma.user.findUnique({ where: { email }, include: { org: { select: { type: true } } } });
  ...
  // KLEAF orgType kullanıcıları ve SUPER_ADMIN denetim app'ine (port 3300) yönlenir; port yanlış app'te değilse otomatik geç
  const orgType = user.org?.type ?? null;
  const redirect =
    user.role === "SUPER_ADMIN" ? "http://localhost:3300/" :
    orgType === "KLEAF" ? "http://localhost:3300/" :
    orgType === "KARBON_BANK" ? "http://localhost:3200/" :
    orgType === "BELEDIYE" ? "http://localhost:3100/" : null;

  const res = NextResponse.json({ ok: true, redirect, orgType });
