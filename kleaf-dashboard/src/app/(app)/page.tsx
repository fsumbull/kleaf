...
export default async function GenelBakis() {
  ...
  const [rows, pending, latest, target, staffAgg] = await Promise.all([
    ...
    getPendingCount(org.id),
    getLatestPeriod(org.id),
    prisma.target.findUnique({ where: { orgId_year: { orgId: org.id, year } } }),
    prisma.facility.aggregate({ where: { orgId: org.id }, _sum: { staffCount: true } }),
  ]);
  ...
  const missing = latest ? await getMissingData(org.id, latest.year, latest.month) : [];
