...
export default async function VeriGirisiPage({ searchParams }: {
  ...
  const { session, org, year: cookieYear } = await getScope();
  ...
  const [facilities, vehicles, yearsRaw] = await Promise.all([
  ...
  return (
    <>
      ...
      <Suspense>
        <VeriGirisiClient
          rows={rows} facilities={facilities} vehicles={vehicles}
