...
export async function POST(req: Request) {
  ...
  for (const r of rows) {
    const existing = await prisma.activityData.findUnique({
      where: { facilityId_year_month_category_vehicleKey: { facilityId: r.facilityId, year: r.year, month: r.month, category: r.category, vehicleKey: r.vehicleId ?? "" } },
      select: { id: true },
    });
    r.mevcut = !!existing;
  }
  ...
  for (const r of rows) {
    ...
    const key = { facilityId: r.facilityId, year: r.year, month: r.month, category: r.category, vehicleKey: r.vehicleId ?? "" };
    ...
    await prisma.activityData.upsert({
      where: { facilityId_year_month_category_vehicleKey: key },
      create: {
