...
const SCOPE_COLORS: Record<number, string> = { 1: "#0c4a33", 2: "#16a34a", 3: "#86efac" };
      ...
      <Chart
        ...
        option={{
          ...
          graphic: [{
            ...
            style: { text: fmtTons(total), fontSize: 24, fontWeight: "bold", fill: "#0c4a33", fontFamily: "var(--font-sg)" },
          }, {
            type: "text", left: "center", top: "56%",
            style: { text: "tCO₂e toplam", fontSize: 11, fill: "#0c4a3380", fontFamily: "var(--font-sg)" },
          }],
...
export function SourceSankey({ rows }: { rows: SankeyRow[] }) {
  ...
  const nodes = [
    ...positives.map((r) => ({ name: r.label, itemStyle: { color: "#4ade80" } })),
    ...[...new Set(positives.map((r) => r.scope))].map((s) => ({
      name: scopeNames[s], itemStyle: { color: SCOPE_COLORS[s] },
    })),
    { name: "Toplam envanter", itemStyle: { color: "#0c4a33" } },
  ];
      ...
      <Chart
        ...
        option={{
          ...
          series: [{
            ...
            label: { fontSize: 11, color: "#0c4a33", fontFamily: "var(--font-sg)" },
